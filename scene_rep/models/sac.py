...
class SACAgent(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Target networks generate bootstrap and SLT targets. Keep their
        # dropout disabled so both targets are stable between optimizer steps.
        ...
        # The MST encoder is intentionally owned by encoder_optimizer only.
        # Giving it a second Adam state here makes SAC and SLT apply unrelated
        # momentum estimates to the same parameters.
    ...
    def update_slt(self, sequence_batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
            ...
            # The flattened states retain sequence-contiguous ordering:
            # [episode_0 t_0..t_T, episode_1 t_0..t_T, ...]. Reusing one
            # rotation per sequence preserves the action-conditioned dynamics.
            obs = augment_observation(obs, group_size=seq_len)

            online_latents = self.encoder(obs)
            online_latents = online_latents.reshape(batch_size, seq_len, -1)

            # Match the original Scene-Rep training pattern: future latent
            # targets are generated by the slowly moving target encoder, not
            # by the same online encoder optimized by this loss.
            with torch.no_grad():
                target_latents = self.target_encoder(obs)
                target_latents = target_latents.reshape(batch_size, seq_len, -1)
            ...
            weighted_loss = float(self.slt_cfg.get("loss_weight", 1.0)) * loss

            # Reuse the encoder's SAC Adam state rather than maintaining a
            # conflicting second optimizer for the shared representation.
            self.encoder_optimizer.zero_grad()
            ...
            metrics["slt_weighted_loss"] = float(weighted_loss.detach().cpu())
            return metrics
